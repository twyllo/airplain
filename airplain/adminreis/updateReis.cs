...
{
    public partial class updateReis : Form
    {
        public updateReis()
        {
            InitializeComponent();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (!(string.IsNullOrEmpty(dcitybtn.Text)
                        ...
                        || string.IsNullOrEmpty(idreis.Text)
                        ))
                {
                    ...
                    dt = conSql.getdata($"select * from public.f_update ('{idreis.Text}', '{dateTimePicker1.Text}', '{dateTimePicker1.Text}', '{dcitybtn.Text}', '{acitybtn.Text}', {idairplane.Text}, {idairline.Text});");

                    if (dt.Rows.Count > 0)
                    {
                        MessageBox.Show("Вы успешно изменили рейс!");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Ошибка изменения рейса: " + ex.Message);
